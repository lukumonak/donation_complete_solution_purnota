...
uploadProduct=async(req,res)=>{
    const {name,type,weight,email,pincode,landmark}=req.body
    
    try{
        const user_id=req.user._id
        const uploadProduct=await UploadProduct.create({name,type,weight,email,pincode,landmark,user_id})
...
const {id}=req.params
    ...
    try{
        const user_id=req.user._id
        const updatepro=await UploadProduct.updateOne({_id:id},{$set:{name,type,weight,email,pincode,landmark}})
